...
int main(void){
    ...
    sort(ALL(lst),[](const vector<ll> &alpha,const vector<ll> &beta){return alpha[0] < beta[0];});
    ll a,b,c;
    a = k / n;
    b = k % n;
    c = a + 1;
    ...
    sort(ALL(lst),[](const vector<ll> &alpha,const vector<ll> &beta){return alpha[1] < beta[1];});
    rep(i,n){
        cout << lst[i][2] << endl;
    }
}